...
# ===== 2. 分析降水变化 =====
# GPCP 的 precip 字段是日均降水率(mm/day)，需先按各月天数换算为月总量(mm/月)，
# 前汛期(4-6月)季总量 = 各月日均率 × 该月天数后求和
df_p = df_p.copy()
df_p['precip_total'] = df_p['precip'] * df_p['time'].apply(lambda t: pd.Period(t, 'M').days_in_month)

df_pre = df_p[df_p['month'].isin([4, 5, 6])]
pre_sum = df_pre.groupby('year')['precip_total'].sum().reset_index()
...
pre_before = pre_sum[before_mask]['precip_total'].mean()
pre_after = pre_sum[after_mask]['precip_total'].mean()

print(f"\n2. 降水变化:")
print(f"   2019前: {pre_before:.1f} mm")
print(f"   2019后: {pre_after:.1f} mm")
...
corr = merged['sst_mean'].corr(merged['precip_total'])
corr_p = stats.pearsonr(merged['sst_mean'], merged['precip_total'])[1]
...
ax1.axvline(x=2019, color='green', linestyle=':', linewidth=2, label='2019(高值期开始)')
ax1.fill_between(yrs[yrs<2019], 25, 29, alpha=0.1, color='blue', label='2019前')
ax1.fill_between(yrs[yrs>=2019], 25, 29, alpha=0.1, color='red', label='2019后')
ax1.set_ylabel('SST (°C)')
ax1.set_title('(a) 南海北部海温年际变化')
ax1.legend(loc='upper left')
ax1.grid(True, alpha=0.3)

# 图2: 2019前后月度降水对比（月总量，mm/月）
ax2 = axes[1]
clim_before = df_p[df_p['year'] < 2019].groupby('month')['precip_total'].mean()
clim_after = df_p[df_p['year'] >= 2019].groupby('month')['precip_total'].mean()
months = range(1, 13)
ax2.plot(months, clim_before.values, 'b-', linewidth=2, marker='o', label='2019前(1982-2018)')
ax2.plot(months, clim_after.values, 'r-', linewidth=2, marker='s', label='2019后(2019-2024)')
...
ax3.bar(yrs, merged['precip_total'].values, color='steelblue', alpha=0.7, label='前汛期降水')
ax3_twin = ax3.twinx()
ax3_twin.plot(yrs, hw_vals, 'r-', linewidth=2, marker='s', label='暖异常月数')
ax3_twin.set_ylabel('暖异常月数', color='red')
ax3.set_xlabel('年份')
ax3.set_ylabel('前汛期降水 (mm)')
ax3.set_title(f'(c) 华南前汛期降水 vs 南海暖异常月频次 (r={corr:.2f})')
...
with open(os.path.join(OUTPUT_DIR, '关键发现.txt'), 'a', encoding='utf-8') as f:
    ...
    f.write(f"  2019前: {pre_before:.1f} mm\n")
    f.write(f"  2019后: {pre_after:.1f} mm\n")
